...
bool CoreConfig::TryParse(const YAML::Node& node)
{
	...
	{
		LOG_E("FAILED_TO_GET_CORE_CONFIG_VALUE(name:{0})", "WindowWidth");
	}

	if (!TryGetValue(node, "WindowHeight", _windowHeight))
	{
		LOG_E("FAILED_TO_GET_CORE_CONFIG_VALUE(name:{0})", "WindowHeight");
	}

	if (!TryGetValue(node, "WindowTitle", _windowTitle))
	{
		LOG_E("FAILED_TO_GET_CORE_CONFIG_VALUE(name:{0})", "WindowTitle");
	}

	if (!TryGetValue(node, "TargetFPS", _targetFPS))
	{
		LOG_E("FAILED_TO_GET_CORE_CONFIG_VALUE(name:{0})", "TargetFPS");
	}
